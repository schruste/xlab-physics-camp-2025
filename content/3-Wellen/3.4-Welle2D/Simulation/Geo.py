from math import pi,sqrt,radians,degrees,cos,sin
...
class Geo:
    ...
    allobjects = []
    allnames = []
    allmaterial = {'Luft':[],'Acrylglas':[],'Sand': [], 'Wachs':[],'Aluminium':[],'Perfekt elektrisch (Dirichlet)':[],'Leitend (Neumann)':[],'Senderantenne':[]}
    refraction = {}
    current_sender = None
    
    def update_refrac_list():
        ### update the dictionaries Geo.allmaterial and Geo.refraction if object in Geo.allobjects has changed/added/removed
        Geo.refraction={}
        Geo.allmaterial = {'Luft':[],'Acrylglas':[],'Sand': [], 'Wachs':[],'Aluminium':[],'Perfekt elektrisch (Dirichlet)':[],'Leitend (Neumann)':[],'Senderantenne':[]}
        for i in Geo.allobjects:
            if i["Typ"] != "Sender":
                Geo.refraction[i["Material"]]=i["Brechungsindex"]
                if i["Material"]==i["Name"]:
                    Geo.allmaterial[i["Material"]] = []
                Geo.allmaterial[i["Material"]].append(i)
        return

    
    current_rpml = pm.scale_domain*0.4
    ngmesh = None
    maxh = None
    ...
    def drawGeo(maxh,domain):
        ...
        if Geo.current_sender != None:
            ...
            x = Geo.current_sender["Bezugspunkt"][0]-0.13
            y = Geo.current_sender["Bezugspunkt"][1]
            ...
            sender_arm1 = Solid2d([(x,y-pm.scale_sender*0.009),
                          ...
                          (x,y-pm.scale_sender*(0.009+pm.mat_thickness)),
                          (x-pm.scale_sender*pm.mat_thickness, y-pm.scale_sender*0.009)],mat="Senderantenne")
            ...
            if Geo.current_sender["Rotationswinkel"] !=0:
                sender.Rotate(Geo.current_sender["Rotationswinkel"],center=(x,y))
        ...
        for material in Geo.refraction :
            ...
            for i in Geo.allmaterial[material]:
                if i["Typ"] == "Block":
                    ...
                    x = i["Bezugspunkt"][0]
                    y = i["Bezugspunkt"][1]
                    obj = Solid2d([(x-pm.scale_lens*0.135,y),
                                  (x+pm.scale_lens*0.135,y),
                                  (x+pm.scale_lens*0.135,y+pm.scale_lens*0.012),
                                   EI((x+pm.scale_lens*0.0675,y+pm.scale_lens*0.012+pm.scale_lens*0.031)),
                                   (x,y+pm.scale_lens*0.031+pm.scale_lens*0.012),
                                  EI((x-pm.scale_lens*0.0675,y+pm.scale_lens*0.012+pm.scale_lens*0.031)),
                                  (x-pm.scale_lens*0.135,y+pm.scale_lens*0.012)],mat=i["Material"])
                    #if i["Rotationswinkel"] != 0:
                    obj.Rotate(-90+i["Rotationswinkel"],center=i["Bezugspunkt"])
                    if cnt == 0:
                        domain_mat = obj
                        cnt = 1
                    else:
                    ...
                    ru = 0.21/sqrt(3)
                    h = sqrt(3)*0.21/2
                    obj = Solid2d([(x-pm.scale_prism*0.21/2, y-pm.scale_prism*(h-ru)),
                                   (x+pm.scale_prism*0.21/2, y-pm.scale_prism*(h-ru)),
                                    (x,y+pm.scale_prism*0.21/sqrt(3))],mat=i["Material"])
                    if i["Rotationswinkel"] != 0:
                        obj.Rotate(i["Rotationswinkel"],center=i["Bezugspunkt"])
                    if cnt == 0:
                        domain_mat = obj
                        cnt = 1
                    else:
    ...
    def sender(typ,name,x,y,rotation,update=False):
        ...
        if typ == "Sender":
            ...
            refrac = 1.57
            pnt1= x
            pnt2= y + pm.scale_lens*0.012
            rad = pm.scale_lens * sqrt(0.135**2+0.012**2)
        ...
        if update == True:
            if Geo.overlapping(name,pnt1,pnt2,rad,refrac):
                for i in Geo.allobjects:
                    if i["Name"] == name:
                        i["Mittelpunkt"]= (pnt1,pnt2)
                        i["Bezugspunkt"]=(x,y)
            ...
            if Geo.same_name(name) and Geo.overlapping(name,pnt1,pnt2,rad,refrac) :
                Geo.allnames.append(name)
                obj = {"Name":name,"Typ":typ,"Bezugspunkt":(x,y),"Radius":rad,"Rotationswinkel":rotation,"Material":mat, "Brechungsindex":refrac, "Mittelpunkt":(pnt1,pnt2)}
                if typ == "Sender":
    ...
    def block(name,x,y,rotation,rotx,roty,mat,refrac,update=False):
        ...
        ### if yes: add new block
            ...
            if Geo.same_name(name) and Geo.overlapping(name,m1,m2,rad,refrac) :
    ...
    def circle(name,x,y,rad,mat,refrac,update=False):
        ...
        ### if yes: add new circle
            ...
            if Geo.same_name(name) and Geo.overlapping(name,x,y,rad,refrac):
    ...
    def overlapping(name,x,y,rad,refrac):
        ...
        if (sqrt(x**2+y**2)+rad >= pm.current_rpml):
    ...
    def printObjects(everything):
        ...
        if everything == True: 
            for i in Geo.allobjects:
                print("Name\t\t\t ",i["Name"],"\nObjekttyp\t\t",i["Typ"])
                if i["Typ"] == 'Kreis':
                    print("Mittelpunkt\t\t",i["Mittelpunkt"])
                    ...
                    print("Bezugspunkt\t\t",i["Bezugspunkt"])   
                    print("Rotationswinkel\t\t",i["Rotationswinkel"])
